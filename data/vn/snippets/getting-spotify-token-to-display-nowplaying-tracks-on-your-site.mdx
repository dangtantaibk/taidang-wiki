...
- Add a redirect URI. This URI will be used to redirect to your local app after the authentication. For example, `http://localhost:3535`.
...
```bash
https://accounts.spotify.com/authorize?client_id=CLIENT_ID&response_type=code&redirect_uri=http://localhost:3535&scope=user-read-currently-playing
```

Remember to use the same redirect URI that you added to your Spotify app.

In my case, it's `http://localhost:3535`.
...
```bash
http://localhost:3535/?code=a1b2c...i9j0
```
...
let data = {
  ...
  code: 'AQB....GemX',
  redirect_uri: 'http://localhost:3535',
}
